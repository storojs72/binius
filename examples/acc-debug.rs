...
use binius_field::{
	...
	BinaryField128b, BinaryField1b, BinaryField64b, BinaryField8b, ExtensionField, Field,
	PackedBinaryField16x8b, PackedBinaryField1x128b, PackedBinaryField8x16b, PackedField,
	TowerField,
};
...
pub struct TransparentPolyOracle<F, FExt>
where
	...
	FExt: ExtensionField<F>,
{
	...
	DisjointProduct {
		val: DisjointProduct<Box<MultivariatePoly<F, FExt>>, Box<MultivariatePoly<F, FExt>>>,
	},
	RingSwitchEqInd {
		val: RingSwitchEqInd<F, FExt>,
	},
	SelectRow {
		val: SelectRow,
	},
	ShiftIndPartialEval {
		val: ShiftIndPartialEval<F>,
	},
	StepDown {
		val: StepDown,
	},
	...
	FExt: ExtensionField<F>,
{
	fn do_something(&self) {
		match *self {
			MultivariatePoly::Constant { ref val } => {
				println!("I'm a Constant and this is my data {:?}", val)
			}
			MultivariatePoly::DisjointProduct { ref val } => {
				println!("I'm Disjoint product and this is my data: {:?}", val)
			}
			MultivariatePoly::RingSwitchEqInd { ref val } => {
				println!("I'm RingSwitchEqInd and this is my data: {:?}", val)
			}
			MultivariatePoly::SelectRow { ref val } => {
				println!("I'm SelectRow and this is my data {:?}", val)
			}
			MultivariatePoly::ShiftIndPartialEval { ref val } => {
				println!("I'm ShiftIndPartialEval and this is my data: {:?}", val)
			}
			MultivariatePoly::StepDown { ref val } => {
				println!("I'm StepDown and this is my data: {:?}", val)
			}
			MultivariatePoly::StepUp { ref val } => {
				println!("I'm StepUp and this is my data: {:?}", val)
			}
			MultivariatePoly::TowerBasis { ref val } => {
				println!("I'm TowerBasis and this is my data: {:?}", val)
			}
			MultivariatePoly::EqIndPartialEval { ref val } => {
				println!("I'm EqIndPartialEval ad this is my data: {:?}", val)
			}
			MultivariatePoly::Powers { ref val } => {
				println!("I'm Powers and this is my data: {:?}", val)
			}
		}
	}
}
...
fn main() {
	...
	// constant
	let constant: MultivariatePoly<F, FExt> = MultivariatePoly::Constant {
		val: Constant {
			n_vars: 10usize,
			value: one,
			tower_level: 10usize,
	...
	// select row
	let select_row: MultivariatePoly<F, FExt> = MultivariatePoly::SelectRow {
		val: SelectRow {
			n_vars: 20usize,
			index: 20usize,
		},
	...
	// disjoint product
	let disjoint_product: MultivariatePoly<F, FExt> = MultivariatePoly::DisjointProduct {
		val: DisjointProduct {
			0: Box::new(constant),
			1: Box::new(select_row),
		},
	};
